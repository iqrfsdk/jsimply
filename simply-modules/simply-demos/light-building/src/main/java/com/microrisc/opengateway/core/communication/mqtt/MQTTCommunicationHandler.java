...
package com.microrisc.opengateway.core.communication.mqtt;

import com.microrisc.opengateway.core.communication.CommunicationHandler;
...
public abstract class MQTTCommunicationHandler implements MqttCallback, CommunicationHandler<MqttMessage> {
    ...
    /** Logger.*/
    private static final Logger logger = LoggerFactory.getLogger(MQTTCommunicationHandler.class);
     ...
     * @param subscribeTopic the MQTT topics to which the client is to be
     * subscribed
     */
    ...
    private void initSubscriber() {
        try {
            ...
            logger.info("MQTT subscriber was created with ClientID : " + clientId);
    ...
    protected void connectToBroker() throws CommunicationHandlerException {
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Subscriber connected to queue at: " + this.mqttBrokerEndPoint);
            }
    ...
    protected void subscribeToBroker() throws CommunicationHandlerException {
        ...
        while(it.hasNext()) {
            ...
            try {
                ...
                logger.info("Subscriber '" + clientId + "' subscribed to topic: " + subscribeTopic);
            } catch (MqttException ex) {
    ...
    protected void publishToBroker(String topic, String payload, int qos, boolean retained)
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Message: " + payload + " to MQTT topic [" + topic
    ...
    protected void publishToBroker(String topic, MqttMessage message)
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Message: " + message.toString() + " to MQTT topic [" + topic
                        + "] published successfully");
            }
        } catch (MqttException ex) {
            String errorMsg
    ...
    public void connectionLost(Throwable throwable) {
        logger.warn("Lost Connection for client: " + this.clientId
    ...
    public void messageArrived(final String topic, final MqttMessage mqttMessage) {
        if (logger.isDebugEnabled()) {
            logger.info("Got an MQTT message '" + mqttMessage.toString() + "' for topic '" + topic
                    + "'.");
        }
    ...
    public void deliveryComplete(IMqttDeliveryToken iMqttDeliveryToken) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Message - '" + message + "' of client [" + client + "] for the topic ("
                    + topic
