/* 
 * Copyright 2014 MICRORISC s.r.o.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.microrisc.simply.iqrf.dpa.v30x.examples.std_per.ram;

import com.microrisc.simply.CallRequestProcessingState;
import com.microrisc.simply.Simply;
import com.microrisc.simply.SimplyException;
import com.microrisc.simply.Network;
import com.microrisc.simply.Node;
import com.microrisc.simply.errors.CallRequestProcessingError;
import com.microrisc.simply.iqrf.dpa.v30x.DPA_SimplyFactory;
import com.microrisc.simply.iqrf.dpa.v30x.devices.RAM;
import com.microrisc.simply.iqrf.types.VoidType;
import java.io.File;

/**
 * Example of using RAM Peripheral - synchronous version.
 * 
 * @author Michal Konopa
 * @author Rostislav Spinar
 */
public class WriteRead {
    // reference to Simply
    private static Simply simply = null;
    
    // prints out specified message, destroys the Simply and exits
    private static void printMessageAndExit(String message) {
        System.out.println(message);
        if ( simply != null) {
            simply.destroy();
        }
        System.exit(1);
    }
    
    // processes NULL result
    private static void processNullResult(RAM ram, String errorMsg, String notProcMsg) 
    {
        CallRequestProcessingState procState = ram.getCallRequestProcessingStateOfLastCall();
        if ( procState == CallRequestProcessingState.ERROR ) {
            CallRequestProcessingError error = ram.getCallRequestProcessingErrorOfLastCall();
            printMessageAndExit(errorMsg + ": " + error);
        } else {
            printMessageAndExit(notProcMsg + ": " + procState);
        }
    }
    
    
    public static void main(String[] args) {
        // creating Simply instance
        try {
            simply = DPA_SimplyFactory.getSimply("config" + File.separator + "Simply.properties");
        } catch ( SimplyException ex ) {
            printMessageAndExit("Error while creating Simply: " + ex.getMessage());
        }
        
        // getting network 1
        Network network1 = simply.getNetwork("1", Network.class);
        if ( network1 == null ) {
            printMessageAndExit("Network 1 doesn't exist");
        }
        
        // getting node 1
        Node node1 = network1.getNode("1");
        if ( node1 == null ) {
            printMessageAndExit("Node 1 doesn't exist");
        }
        
        // get access to RAM peripheral
        RAM ram = node1.getDeviceObject(RAM.class);
        if ( ram == null ) {
            printMessageAndExit("RAM doesn't exist or is not enabled");
        }
        
        // reading 5 bytes from RAM peripheral 
        short[] readData = ram.read(0x00, 0x05);
        if ( readData == null ) {
            processNullResult(ram, "Getting RAM bytes failed", 
                    "Getting RAM hasn't been processed yet"
            );
        }
        
        System.out.print("Reading before writing 5 bytes to the RAM: ");
        for ( short dataItem : readData ) {
            System.out.printf("%1$02X ", dataItem); 
        }
        System.out.println();
        
        VoidType result = ram.write(0x00, new short[] { 0x01, 0x02, 0x03, 0x04, 0x05 } );
        if ( result == null ) {
            processNullResult(ram, "Writing RAM bytes failed", 
                    "Writing RAM bytes hasn't been processed yet"
            );
        }
        System.out.println("5 bytes written to the RAM");
        
        // reading 5 bytes from RAM peripheral 
        readData = ram.read(0x00, 0x05);
        if ( readData == null ) {
            processNullResult(ram, "Getting RAM bytes failed", 
                    "Getting RAM bytes hasn't been processed yet"
            );
        }
        
        System.out.print("Reading after writing 5 bytes to the RAM: ");
        for ( short dataItem : readData ) {
            System.out.printf("%1$02X ", dataItem); 
        }
        System.out.println();
        
        // end working with Simply
        simply.destroy();
    }
}
