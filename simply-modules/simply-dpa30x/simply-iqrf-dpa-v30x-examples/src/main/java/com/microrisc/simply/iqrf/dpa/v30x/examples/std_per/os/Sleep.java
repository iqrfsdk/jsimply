...
package com.microrisc.simply.iqrf.dpa.v30x.examples.std_per.os;
...
public class Sleep {
    ...
    private static void sleepEg2(OS os){       
       // time in ms; wakeup on PIN negative change; run calibration; 
       // green LED flash after wake up; wakeup on PIN positive change, use 32ms unit, deep sleep
        SleepInfo sleepInfo = new SleepInfo(5*1000, true, true, true, true, true, true);
        VoidType sleepResult = os.sleep(sleepInfo);
        if (sleepResult == null) {            
            CallRequestProcessingState procState = os.getCallRequestProcessingStateOfLastCall();
            if ( procState == ERROR ) {
